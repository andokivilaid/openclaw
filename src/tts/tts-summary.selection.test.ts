...
module.exports = {
  ...
  register(api) {
    api.registerProvider({
      id: ${JSON.stringify(provider)}, label: "TTS selection", auth: [],
      ...
      const cfg: OpenClawConfig = {
        agents: {
          defaults: {
            ...
            model: { primary: options.primary ?? `${provider}/plain` },
            models: {
              [`${provider}/entry`]: { alias: "fast" },
              ...(options.literalRow === `${provider}/entry`
                ? { [`${provider}/${provider}/entry`]: { alias: "literal" } }
                : {}),
            },
          },
          entries: {
            main: {
              model: { primary: `${provider}/agent-model` },
              models: { [`${provider}/agent-model`]: { alias: "fast" } },
            },
          },
        },
        models: {
